...
for( const uas of userAgents['desktop']){
	if(uas['ua'].includes('Intel Mac OS X')){
		newUserAgents['desktop'].push({
			'ua': uas['ua'],
			'vp': {
				width: 1440,
				height: 900,
	...
	else if(uas['ua'].includes('Windows NT 10.0; Win64; x64')){
		newUserAgents['desktop'].push({
			'ua': uas['ua'],
			'vp': {
				width: 1920,
	...
	else if(uas['ua'].includes('X11; Linux x86_64')){
		newUserAgents['desktop'].push({
			'ua': uas['ua'],
			'vp': {
				width: 1920,
...
console.log(JSON.stringify(newUserAgents));

const fs = require('fs');


try{
	fs.writeFileSync('uas.json', JSON.stringify(newUserAgents), 'utf-8');
} catch(e){
	throw e;
}
